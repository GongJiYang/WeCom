...
1. **Clawdbot** 已安装并可运行 Gateway（本机或服务器）。
...
3. **Token**：自定义字符串，用于签名验证（与 Clawdbot 配置中 `webhookToken` 一致）。
4. **EncodingAESKey**：点击「随机获取」或使用已有 43 位密钥，用于消息加解密（与 Clawdbot 配置中 `encodingAESKey` 一致）。
...
在 Clawdbot 项目根目录或已安装 CLI 的环境下执行：
...
向导会写入 `~/.clawdbot/clawdbot.json`（或 `$CLAWDBOT_STATE_DIR/clawdbot.json`）。

### 方式 B：手动编辑配置文件

编辑 `~/.clawdbot/clawdbot.json`，在 `channels` 下增加 `wecom` 节点，例如：
...
- **scripts/verify-decrypt.mjs**：独立于 Clawdbot 的解密/校验脚本，可用于用企业微信官方样例或自有密文做加解密联调。
...
- 确认 **URL**、**Token**、**EncodingAESKey** 与 Clawdbot 中 `webhookToken`、`encodingAESKey` 完全一致（含大小写、无多余空格）。
...
- 确保已配置 AI 模型与鉴权（如 ZAI/OpenAI 等），并已执行 `clawdbot onboard` 或手动配置好对应 auth；否则会报「No API key found for provider」等错误。
